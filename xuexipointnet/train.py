...
if __name__ == "__main__":
    # env_dist = os.environ
    # dataset_path = env_dist.get('D:/三维点云/11/pointnet代码/Pointnet_Pointnet2_pytorch/data')
    ...
    print("device:", device)
    print("Loading train dataset...")
    train_data = PointNetDataset("D:/三维点云/11/pointnet代码/Pointnet_Pointnet2_pytorch/data/modelnet40_normal_resampled", train=0)
    train_loader = DataLoader(train_data, batch_size=batch_size, shuffle=True)
    print("Loading valid dataset...")
    val_data = PointNetDataset("D:/三维点云/11/pointnet代码/Pointnet_Pointnet2_pytorch/data/modelnet40_normal_resampled", train=1)
    val_loader = DataLoader(val_data, batch_size=batch_size, shuffle=True)
    print("Set model and optimizer...")
    model = PointNet().to(device=device)
